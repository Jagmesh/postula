...
function prepareInstructions(): string {
  return (
    'Отправить <b>Гифку + Текст</b> можно несколькими способами:\n\n' +
    '<blockquote>' +
    '- Отправь <b>видео с выключенным звуком</b> <i>(нужно тыкнуть на значок звука перед отправкой)</i> и с текстом к нему;\n\n' +
    '- Отправь <b>гифку без подписи</b>, а затем <b>отредактируй</b> пост, добавив туда текст;\n\n' +
    '- Отправь гифку как файл <i>[так тоже можно, но там дрочно добавлять текcт, придется поковыряться]</i>;\n\n' +
    '- <i>[с ПК]</i> Тыкни правой кнопкой мыши на гифку и выбери "Отправить GIF с подписью"' +
    '</blockquote>'
  );
}
